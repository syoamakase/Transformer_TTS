...
def main(hp, args):
    ...
    for wavpath in tqdm.tqdm(wav_files, desc='preprocess wav to mel'):
        ...
        energy_path = os.path.join(args.out_path, os.path.basename(wavpath.replace('.wav', '_energy.npy')))
        f0_path = os.path.join(args.out_path, os.path.basename(wavpath.replace('.wav', '_f0.npy')))
        np.save(melpath, mel.squeeze(0).transpose(0,1).numpy())
        if args.fastspeech2:
            np.save(energy_path, energy.squeeze(0).numpy())
            np.save(f0_path, f0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--fastspeech2', action='store_true',
                        help="save f0, energy")
