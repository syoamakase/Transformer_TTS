...
class Mel2MagnitudeDataset(Dataset):
    def __init__(self, csv_file):                                   
        """                                                                     
        Args:                                                                   
            csv_file (string): Path to the csv file with annotations.                  
    ...
    def __getitem__(self, idx): 
        mel_name = self.landmarks_frame.loc[idx, 0]
        mag_name = self.landmarks_frame.loc[idx, 1]
 
        if '.npy' in mel_name:
            mel_input = np.load(mel_name)
            mag_output = np.load(mag_name)
            assert mel_input.shape[0] == hp.mel_dim or mel_input.shape[1] == hp.mel_dim, '{} does not have strange shape {}'.format(mel_name, mel_input.shape)
            if mel_input.shape[1] != hp.mel_dim:
                mel_input = mel_input.reshape(-1, hp.mel_dim)
            mel_length = mel_input.shape[0]
        ...
        sample = {'mel_input':mel_input, 'mag_output':mag_output, 'mel_length': mel_length}
...
def collate_fn_mel2magnitude(batch):
    ...
    if isinstance(batch[0], collections.abc.Mapping):
        ...
        mag_output = [d['mag_output'] for d in batch]
        mel_length = [d['mel_length'] for d in batch]

        mel_input = _pad_mel(mel_input)
        mag_output = _pad_mel(mag_output)

        return torch.FloatTensor(mel_input), torch.FloatTensor(mag_output), torch.LongTensor(mel_length)
    ...
    if isinstance(batch[0], collections.abc.Mapping):
        ...
        if hp.is_multi_speaker:
            if hp.spk_emb_type == 'x_vector':
                return torch.LongTensor(text), torch.FloatTensor(mel_input), torch.LongTensor(pos_text), torch.LongTensor(pos_mel), torch.LongTensor(text_length), torch.LongTensor(mel_length), torch.FloatTensor(stop_token), torch.FloatTensor(spk_emb)
            ...
            return torch.LongTensor(text), torch.LongTensor(mel_input), torch.LongTensor(pos_text), torch.LongTensor(pos_mel), torch.LongTensor(text_length), torch.LongTensor(mel_length), torch.FloatTensor(stop_token), None
